...
int StreamConfig_example(HANDLE hDevice)
{
    ...
    sendChars = LJUSB_BulkWrite(hDevice, UE9_PIPE_EP1_OUT, sendBuff, sendBuffSize);
...
int StreamStart(HANDLE hDevice)
{
    ...
    //Sending command to UE9
    sendChars = LJUSB_BulkWrite(hDevice, UE9_PIPE_EP1_OUT, sendBuff, 2);
    if(sendChars < 2)
    {
        if(sendChars == 0)
...
int StreamData_example(HANDLE hDevice, ue9CalibrationInfo *caliInfo)
{
    ...
    for (i = 0; i < numDisplay; i++)
    {
        for(j = 0; j < numReadsPerDisplay; j++)
        {
            ...
            //Reading response from UE9
            recChars = LJUSB_BulkRead(hDevice, UE9_PIPE_EP2_IN, recBuff, 192*readSizeMultiplier);
...
int flushStream(HANDLE hDevice)
{
    ...
    //Reading response from UE9
    recChars = LJUSB_BulkRead(hDevice, UE9_PIPE_EP1_IN, recBuff, 2);
    if(recChars < 2)
    {
        if(recChars == 0)
    ...
    {
        printf("Error : read buffer has wrong command bytes (flushStream).\n");
        return -1;
    }
...
int doFlush(HANDLE hDevice)
{
    ...
    StreamStop(hDevice);
    flushStream(hDevice);
    LJUSB_BulkRead(hDevice, UE9_PIPE_EP2_IN, recBuff, 128);
