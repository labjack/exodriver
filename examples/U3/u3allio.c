...
int allIO(HANDLE hDevice, u3CalibrationInfo *caliInfo, int isDAC1Enabled)
{
    ...
    sendBuff = malloc(14*sizeof(uint8));	//Creating an array of size 14
    recBuff = malloc(10*sizeof(uint8));   //Creating an array of size 10
    ...
    //Sending command to U3
    if( (sendChars = LJUSB_BulkWrite(hDevice, U3_PIPE_EP1_OUT, sendBuff, 14)) < 14)
    {
        if(sendChars == 0)
    ...
    {
        //Sending command to U3
        if( (sendChars = LJUSB_BulkWrite(hDevice, U3_PIPE_EP1_OUT, sendBuff, sendSize)) < sendSize)
        {
            if(sendChars == 0)
        ...
        //Reading response from U3
        if( (recChars = LJUSB_BulkRead(hDevice, U3_PIPE_EP2_IN, recBuff, recSize)) < recSize)
        {
            if(recChars == 0)
            {
